...
in
{
  options.programs.whisperDictate.groqApiKeyFile = lib.mkOption {
    ...
    description = ''
      Runtime path to a file containing the Groq API key. When set, the
      whisper-dictate script will read GROQ_API_KEY from this file at runtime
      if GROQ_API_KEY is not already present in the environment.
    '';
  };
